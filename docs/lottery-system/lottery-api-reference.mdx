...
- **응답**:
  ```json
  {
    ...
    "prizeWinners": [ // [필수] 경품별 당첨자 정보
      {
        "prizeName": "S석", // [필수] 경품명
        "winnerCount": 20, // [필수] 당첨자 수
        "winners": [ // [필수] 당첨자 목록 (마스킹 처리)
          {
...
### 1. 앱 API 추가 (2025-08-19)
- 모바일 앱용 추첨 결과 조회 API 구현
- 인증 불필요 (publicProcedure)
- 개인정보 자동 마스킹 처리
- 경품별 당첨자 그룹화 기능

### 2. OrderBy 필드명 수정
- `LotteryHistOrderBy`: totalCandidates → totalEntries (실제 DB 필드명과 일치)
- `LotteryResultOrderBy`: openStartAt/openEndAt → displayStartAt/displayEndAt

### 3. 코드 개선
- paginationSchema 활용으로 중복 제거
- createProgressStatusFilter 함수로 표시 기간 필터링 통일
- 네이밍 규칙 통일 (OutputItemSchema/DetailOutputSchema 패턴)
...
- 400: 입력 데이터 검증 실패
- 403: 권한 부족 또는 비즈니스 로직 제약 (보존 기간 만료 포함)
- 404: 리소스를 찾을 수 없음
...
- `FDE-L-203`: LOTTERY_RETENTION_EXPIRED - 추첨 보존 기간 만료